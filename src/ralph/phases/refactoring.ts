...
export class RefactoringPhase extends BasePhase {
  readonly name = 'refactoring' as const;
  readonly icon = '🧹';
  ...
  async execute(context: PhaseContext): Promise<PhaseResult> {
    ...
    if (!output.success) {
      const error = extractError(output.result) || 'No REFACTORING_COMPLETE marker found';
