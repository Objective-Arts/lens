...
The pipeline has five stages — **Design**, **Build**, **Refine**, **Review**, **Verify** — connected by a **Learn** loop that feeds late-stage findings back to early stages. Nine named patterns recur throughout:

- **Gate** — a mechanical pass/fail check. No AI, no judgment, no override.
- **Contract** — an abstract quality type assigned at Design, enforced at Build.
- **Budget** — a complexity constraint: Refine and Review can't add more than they remove.
- **Canary** — known violations planted before review; if the reviewer misses them, the review is void.
- **Evidence** — a structured checklist proving every item was examined; the machine rejects incomplete reviews.
- **Vote** — multi-model disagreement resolution across three independent reviewers.
- **Loop** — retry with specifics until complete or max attempts.
- **Lesson** — a finding captured so the same defect class is prevented next run.
- **Rubric** — a domain checklist auto-detected from project signals.

**Design.** `create-plan` designs the approach, identifying scope, files, risks, and decision points. The plan must include specific sections — files, functions with size estimates, types, dependencies, invariants, security measures, test cases, and concrete work items — or the stage fails validation. `structure-first` then defines data structures and interfaces before any implementation, assigning **Contracts** to boundaries so quality is designed in, not bolted on.

**Build.** `implement-plan` writes the code one unit at a time. For each unit, it rereads the most relevant skill, writes the code, then runs the type checker before starting the next unit. This catches cascading type errors at the source instead of discovering them three files later. A **Loop** retries until all work items complete. Then a **Gate** verifies compilation and existing tests, and a live test confirms the application actually starts and serves responses.

**Refine.** `refactor-check-fix` enforces structural constraints under a **Budget**: functions under 30 lines, files under 300 lines, low branching complexity, clear naming, single responsibility. It produces **Evidence** — every exported function evaluated for name clarity, every function for single responsibility, every literal for unexplained magic values — with pass/fail verdicts. The machine validates completeness: if the file has 14 exported functions and the review only covers 9, the review is rejected. `dedupe-fix` consolidates duplicated code, searching for identical logic across files and extracting it to shared utilities.

**Review.** `gemini-fix` sends the code to Gemini for an external review of code and product quality. Before the review, the system inserts a **Canary** — known violations planted in the code. If the reviewer misses them, the review is discarded and rerun. `codex-fix` sends the code to Codex for a second independent review. Having three models (Claude, Gemini, Codex) review the same code catches blind spots. A **Vote** resolves disagreements. A **Gate** runs Qodana static analysis, then a live test re-verifies the application still starts. `adversarial-security-review` examines the code from an attacker's perspective — injection points, trust boundary violations, path traversal, secrets in process listings, data integrity gaps. Every issue must be fixed; there is no escape hatch. `ai-smell-fix` removes AI-generated antipatterns: single-use helpers, defensive null checks where null is impossible, speculative configuration, comment spam. All findings feed the **Lesson** system. A **Gate** runs the test suite.

**Verify.** `write-tests-run` writes and executes tests covering happy paths, error cases, edge cases, and adversarial scenarios like corrupted data recovery, lock contention, interrupted writes, and path escape. Before running, it audits existing test mocks: any test that mocks the module under test — rather than its dependencies — is deleted and rewritten. `final-eval-check` — always last — runs Codex and Gemini as independent evaluators with **Evidence** checklists, scoring production readiness across five categories: deployability, reliability, security, test coverage, and operational hygiene. If the score is too low, a **Loop** spawns a fix agent, applies corrections, and rescores. Findings become **Lessons** for future runs. A final **Gate** re-runs the full test suite.

**Learn.** The sixth element operates across runs. Late stages — Review and Verify — write **Lessons** to files. Early stages — Design and Build — read those **Lessons** on future runs. A defect caught once is prevented forever.
...
The Lesson pattern operates across runs, not within them. Late stages — Review and Verify — write their findings to lesson files. Early stages — Design and Build — read those lesson files on future runs.
...
The Verify stage adds a second feedback channel. External evaluators score production readiness independently. Their findings are classified: code patterns become lessons that feed early stages; pipeline and tooling suggestions become proposals that surface during Design. Findings like missing health checks or absent `.gitignore` files automatically influence the next pipeline run without manual routing.
...
Lens ships as `@objective-arts/lens`, a Node.js command-line tool installable via npm. It has 75 canon skills across 29 categories, 14 composable profiles, 33 workflow skills, a five-stage pipeline (Design → Build → Refine → Review → Verify) built on nine named patterns, and an autonomous implementation system.
...
Lens doesn't replace that judgment. It encodes it. Seventy-five skills, each capturing one expert's hard-won perspective. Nine patterns — Gate, Contract, Budget, Canary, Evidence, Vote, Loop, Lesson, Rubric — ensuring the expertise is applied, not merely available. A pipeline that treats quality as a manufacturing process, not a hope. And a system that learns from its own mistakes.
