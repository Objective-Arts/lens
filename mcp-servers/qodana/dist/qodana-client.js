...
export class QodanaClient {
    ...
    detectAllLinters(projectDir) {
        ...
        const hasRootFile = (patterns) => patterns.some(p => rootFiles.some(f => f.match(new RegExp(p, 'i'))));
        const hasAnyFile = (patterns) => patterns.some(p => allFiles.some(f => f.match(new RegExp(p, 'i'))));
