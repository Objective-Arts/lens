...
export function registerCanonCommands(program: Command): void {
  ...
  canonCmd.command('install <skill>').description('Install a skill')
    .option('-p, --project <path>', 'Project path', process.cwd())
...
function validateName(name: string): boolean {
  if (!isValidName(name)) {
    console.log(chalk.red(`Invalid name: ${getNameValidationError(name, 'skill')}`));
    return false;
  }
  return true;
}
...
function handleInstall(skill: string, options: { project: string; force?: boolean }): void {
  if (!validateName(skill)) return;
  const projectPath = validatePath(options.project);
  if (!projectPath) return;

  const result = copySkill(skill, projectPath, { force: options.force });
  console.log(result.success
    ? chalk.green(`${result.message}\nInstalled to: ${projectPath}/.claude/skills/${skill}/`)
