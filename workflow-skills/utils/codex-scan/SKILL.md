...
Run the review-bot.sh script bundled with the codex-fix skill:

```bash
SKILL_DIR="$(dirname "$(readlink -f workflow-skills/workflow/codex-fix/SKILL.md)" 2>/dev/null || echo workflow-skills/workflow/codex-fix)"
...
- Applying fixes (use /codex-fix for that)
