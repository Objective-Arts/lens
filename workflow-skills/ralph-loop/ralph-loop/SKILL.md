---
name: ralph-loop
description: Autonomous PRD implementation with 5-stage quality pipeline (Design → Build → Refine → Review → Verify). Keeps going until done.
---

# /ralph-loop [prd-file] [--max N] [--resume]

Autonomous loop that implements PRD items through a 5-stage quality pipeline (Design → Build → Refine → Review → Verify).
...
Ralph exists to produce code that looks like it was written by a skilled human engineer, not generated by AI. This is the north star for every stage.
...
PER PRD ITEM (Design → Build → Refine → Review → Verify):
  Design   📝  plan + structure-first → Analyze requirements, design types
  Build    🛠️  implement              → Write code
  Refine   🧹  refactor + dedupe      → Clean up, consolidate
